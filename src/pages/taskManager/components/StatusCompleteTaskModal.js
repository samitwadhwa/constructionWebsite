...
export default function StatusCompleteModal({ open, setOpen,taskId }) {
    const handleSubmit = (e) => {
        axios.put(`${process.env.REACT_APP_BASE_URL}/api/task-manager/task-completed/${taskId}`,{}, {headers: { authorization: `Bearer: ${token}` } }).then((result) =>{
            console.log(result)
            setOpen(false);
            ...
            <Container maxWidth='sm' fixed>
                ...
                >
                    ...
                    <Typography variant="h6" id="Name" key="name" sx={{ mb: 1 }}>
                        Do you want to change the status of this task ?<br/>Doing this will permanently change the status of the task from pending to completed.
                    </Typography>
                    </div>
